class World {
  ...
  static load(info) {
    ...
    world.buildingsWidth = info.buildingsWidth;
    world.buildingMinLength = info.buildingMinLength;
    world.spacing = info.spacing;
    world.treeSize = info.treeSize;
    world.envelopes = info.envelopes.map((e) => Envelope.load(e));
    world.roadBorders = info.roadBorders.map((b) => new Segment(b.p1, b.p2));
    world.buildings = info.buildings.map((bu) => Building.load(bu));
    world.trees = info.trees.map(
      (tree) => new Tree(tree.center, info.treeSize)
    );
    world.laneGuides = info.laneGuides.map((lg) => new Segment(lg.p1, lg.p2));
  ...
  #generateBuildings() {
    ...
    for (const seg of this.graph.segments) {
      tmpEnvelopes.push(
        new Envelope(
          seg,
          this.roadWidth + this.buildingsWidth + this.spacing * 2,
          this.roadRoundness
        )
      );
    }
